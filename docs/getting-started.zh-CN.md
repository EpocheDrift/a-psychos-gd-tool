...
应用启动后会先展示一份已经在渲染的示例工程。左边是节点图，右边是画板，浮动的
**layers** 面板决定你当前正在编辑哪一个图层的节点图。
...
在 **layers** 面板点击 `+`。新图层会自动变成当前图层，而且已经自带一个透明的
**Output** 节点。先点击其他示例图层的实心圆可见性按钮，把它们隐藏起来，这样
画面里只留下你的新图层。

如果想让下面的图层透出来，就保留 Output 的 **transparent**。如果想做一张独立
海报，就关闭 **transparent**，再选择一个背景色。
...
新图层默认透明，所以底下的示例图层还会显示。把旧图层隐藏，或者关闭新图层
Output 的 **transparent** 并设置背景色。
