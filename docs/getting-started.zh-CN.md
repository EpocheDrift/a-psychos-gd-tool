...
人类和 Agent 编辑同一份文档的共享工作台。使用 Agent 时不需要继续运行 5173。可见的
5199 窗口只使用一个方便的初始大小，之后会跟随 Chrome 的真实窗口：你可以像普通应用
一样调整窗口或浏览器缩放。左右区域之间的分隔线可以拖动或用键盘调整；窗口较窄时会
...
仍然是下载出来的工程文件。5199 尤其需要注意：Companion 使用临时的隔离浏览器
Context；如果想下次会话继续编辑，请在关闭 Companion 或它的 Chrome 窗口前执行
**save project**，不要把这份临时浏览器存储当成跨会话备份。
...
同一份文档。调整窗口、浏览器缩放或工作台分隔线只会改变 UI 的显示方式，不会改变
Frame 像素尺寸，也不会改变 MCP 返回的准确 revision preview。
